...
from ..geometry import Point, Tx
from ..ir import NCNet
from .. import __version__
from .net_terminal import NetTerminal
from .._utils import export_to_all

# Which KiCad version the output targets; ``render`` sets this so the tiny
# per-version format toggles (KiCad 8) apply. Kept module-level because the
# lib-symbol writer is a plain function with no tool argument.
TARGET_TOOL = "kicad9"
...
def _find_power_lib():
    """Locate ``power.kicad_sym``: prefer the vendored copy, else a KiCad install.

    The vendored copy keeps the tool self-contained (no KiCad library access
    required). The environment fallback lets a user point at a newer library.
    """
    here = os.path.dirname(__file__)
    vendored = os.path.normpath(os.path.join(here, "..", "data", "power.kicad_sym"))
    ...
    for var in (
        ...
        if d:
            candidates.append(os.path.join(d, "power.kicad_sym"))
    candidates += [
...
def init_power_symbol_data():
    ...
    # Read the power symbols (GND, +3V3, ...) so nets whose name matches one can
    # be emitted as proper KiCad power symbols. Absent the library, such nets
    # simply fall back to plain net labels.
    path = _find_power_lib()
    if not path:
    ...
    pwr_symbol_names = set(pwr_symbol_sexp_dict.keys())
...
def part_to_lib_symbol_definition(part):
    ...
    pin_numbers = (
        ["pin_numbers", "hide"]
        if TARGET_TOOL == "kicad8"
        else ["pin_numbers", ["hide", "yes"]]
    )
    ...
    # KiCad 9/10 carry an embedded-fonts flag on each lib symbol; KiCad 8 omits it.
    if TARGET_TOOL != "kicad8":
...
def node_to_sexp_schematic(node, uuid_path, sheet_tx=Tx(), version=20230409):
    ...
    from .backend import RenderContext
    from .kicad_backend import Kicad9Backend
