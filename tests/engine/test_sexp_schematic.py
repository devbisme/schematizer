...
from schematizer.geometry import BBox, Point, Tx
from schematizer.engine.sexp_schematic import part_to_lib_symbol_definition

sexp_schematic = importlib.import_module("schematizer.engine.sexp_schematic")
...
class TestGenUuid:
    ...
    def test_namespace_matches_gen_netlist(self):
        ...
        from schematizer.engine.sexp_schematic import _NAMESPACE_UUID
...
class TestLibrarySymbolDefinition:
    ...
    def test_kicad8_uses_legacy_pin_number_syntax(self):
        ...
        # One writer serves every KiCad version; the target is selected by the
        # module-level TARGET_TOOL that render() sets.
        saved = sexp_schematic.TARGET_TOOL
        sexp_schematic.TARGET_TOOL = "kicad8"
        try:
            symbol_def = part_to_lib_symbol_definition(part)
        finally:
            sexp_schematic.TARGET_TOOL = saved
